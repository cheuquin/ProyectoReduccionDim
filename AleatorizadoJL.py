...
def JL (wi, wj, vi, vj, epsilon):
{
    exito = 0
    proyected = la.norm(wi - wj ,2)**2
    minim = (1-epsilon)*la.norm(vi - vj, 2)**2
    maxim = (1+epsilon)*la.norm(vi - vj, 2)**2
    if ((proyected >= minim) && (proyected <= maxim)):
        return 1
}
...
for i in range(n):
    for j in range(n):
            exito += JL(DimReducedData[i],DimReducedData[j],espectros[i],espectros[j],epsilon)
